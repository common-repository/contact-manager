...
$protocol = (stristr($protocol, 'https') ? 'https' : 'http');
if ($string != '') {
$string = trim(strip_tags($string));
$string = str_replace(' ', '-', $string);
if ((substr($string, 0, 1) != '.') && (!strstr($string, 'http://')) && (!strstr($string, 'https://'))) {
$strings = explode('/', $string);
if (strstr($strings[0], '.')) { $string = $protocol.'://'.$string; }
else { $string = $protocol.'://'.$_SERVER['SERVER_NAME'].'/'.$string; } }
...
if (strings[0].indexOf(".") >= 0) { string = "<?php echo $protocol; ?>://"+string; }
else { string = "<?php echo $protocol; ?>://<?php echo $_SERVER['SERVER_NAME']; ?>/"+string; } }
