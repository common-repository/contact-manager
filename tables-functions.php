...
return '<div class="main-row-actions">'.$row_actions.'</div>'; }
...
case 'sender_members_areas_modifications': $table_td = trim($data); if (substr($table_td, -1) == ',') { $table_td = substr($table_td, 0, -1); }
$table_td = str_replace("\n", "<br />", $table_td); break;
...
echo '<div class="tablenav-pages" style="float: right;"><span class="displaying-num">'.$n.' '.($n <= 1 ? $singular : $plural).'</span><span class="pagination-links">
'.($_GET['paged'] == 1 ? '<span class="tablenav-pages-navspan button disabled" aria-hidden="true">&laquo;</span> <span class="tablenav-pages-navspan button disabled" aria-hidden="true">&lsaquo;</span>'
 : '<a class="first-page button" title="'.__('Go to the first page', 'contact-manager').'" href="'.$url.'&amp;paged=1">&laquo;</a>
<a class="prev-page button" title="'.__('Go to the previous page', 'contact-manager').'" href="'.$url.'&amp;paged='.$prev_paged.'">&lsaquo;</a>').'
<span class="paging-input" id="paging-input-'.$location.'">'.($location == 'top' ? '<input type="hidden" name="old_paged" value="'.$_GET['paged'].'" /><input class="current-page" title="'.__('Current page', 'contact-manager').'" type="text" name="paged" id="paged" value="'.$_GET['paged'].'" size="2" onfocus="this.value = \'\';" onblur="if (this.value == \'\') { this.value = '.$_GET['paged'].'; }" onkeyup="this.value = kleor_format_integer(this.value); this.form.paged2.value = this.value;" onchange="this.value = kleor_format_integer(this.value); if ((this.value == \'\') || (this.value == 0)) { this.value = '.$_GET['paged'].'; } if (this.value > '.$max_paged.') { this.value = '.$max_paged.'; } this.form.paged2.value = this.value; if (this.value != '.$_GET['paged'].') { window.location = \''.$url.'&amp;paged=\'+this.value; }" />' : $_GET['paged']).'</span> '.__('of', 'contact-manager').' <span class="total-pages">'.$max_paged.'</span>
'.($_GET['paged'] == $max_paged ? '<span class="tablenav-pages-navspan button disabled" aria-hidden="true">&rsaquo;</span> <span class="tablenav-pages-navspan button disabled" aria-hidden="true">&raquo;</span>'
 : '<a class="next-page button" title="'.__('Go to the next page', 'contact-manager').'" href="'.$url.'&amp;paged='.$next_paged.'">&rsaquo;</a>
<a class="last-page button" title="'.__('Go to the last page', 'contact-manager').'" href="'.$url.'&amp;paged='.$max_paged.'">&raquo;</a>').'
</span></div>'; }
