...
global $wpdb;
$GLOBALS['kleor_shortcodes_cache'] = 'no';
...
if (is_admin()) { $message[$field] = (isset($message[$field]) ? stripslashes(kleor_do_shortcode($message[$field])) : ''); }
...
if (!function_exists('kleor_generate_password')) { include_once contact_path('libraries/passwords-functions.php'); }
...
foreach ($add_message_fields as $field) {
if (is_admin()) { $message[$field] = stripslashes(kleor_do_shortcode($original[$field])); }
else { $message[$field] = contact_form_data($field); } }
...
contact_subscribe_to_autoresponder($message['sender_autoresponder'], $message['sender_autoresponder_list'], $message); }

if ($message['message_custom_instructions_executed'] == 'yes') {
$instructions = kleor_format_instructions($message['message_custom_instructions']);
