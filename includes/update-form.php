...
if ($_POST['name'] != '') {
if (!$is_category) { if (isset($_POST['submit'])) { $results = $wpdb->query("UPDATE ".$wpdb->prefix."contact_manager_".$table_slug." SET name = '".str_replace(array("\\", "'"), array("", "''"), $_POST['name'])."' WHERE id = ".$_GET['id']); } }
else {
...
if (kleor_get_option('affiliation_manager')) {
...
$_POST['commission_amount'] = round((float) $_POST['commission_amount'], 2); if ($_POST['commission_amount'] <= 0) { $_POST['commission_amount'] = 0; }
...
if (($_POST['referrer2'] == '') && ((!isset($_POST['referrer2_emptied'])) || ($_POST['referrer2_emptied'] != 'yes')) && ($_POST['referrer'] != '') && (kleor_get_option('affiliation_manager'))) {
...
$_POST['commission2_amount'] = round((float) $_POST['commission2_amount'], 2); if ($_POST['commission2_amount'] <= 0) { $_POST['commission2_amount'] = 0; }
...
if (!isset($_GET['id'])) {
if (($_POST['referring_url'] == '') && ((!isset($_POST['referring_url_emptied'])) || ($_POST['referring_url_emptied'] != 'yes'))) { $_POST['referring_url'] = (isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : ''); }
...
kleor_do_shortcode(str_replace(array('[message id', '[sender id'), array('{message id', '{sender id'), $_POST[$field]))); }
...
include_once contact_path('libraries/autoresponders-functions.php');
foreach (contact_autoresponders() as $key => $value) {
if ((isset($value['update_function'])) && (function_exists($value['update_function']))) {
$function = $value['update_function']; $options = $function($options); } }
foreach (array(
'automatic_display_maximum_forms_quantity',
'maximum_messages_quantity') as $field) { if ((!isset($_POST['submit'])) && ($_POST[$field] === 'unlimited')) { $_POST[$field] = ''; } }
if (isset($_POST['submit'])) { kleor_update_option('contact_manager', $options); }
$can_edit_plugins = (current_user_can('edit_plugins'));
foreach ($other_options as $field) {
if ((!$can_edit_plugins) && (substr($field, -19) == 'custom_instructions')) { $_POST[$field] = kleor_get_option('contact_manager_'.$field); }
if ((!isset($_POST[$field])) || ($_POST[$field] == '')) { $_POST[$field] = $initial_options[$field]; }
if (isset($_POST['submit'])) { kleor_update_option(substr('contact_manager_'.$field, 0, 64), $_POST[$field]); } }
...
if (!isset($_POST['submit'])) { $values = $_POST; }