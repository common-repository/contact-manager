...
case 'url': if ($atts['size'] == '') { $atts['size'] = '60'; } }
if ($atts['type'] == 'file') {
$extensions = array_unique(preg_split('#[^a-z0-9]#', strtolower($atts['extensions']), 0, PREG_SPLIT_NO_EMPTY));
$unauthorized_extensions = array_map('strtolower', array_map('trim', (is_array(CONTACT_MANAGER_UNAUTHORIZED_EXTENSIONS) ? CONTACT_MANAGER_UNAUTHORIZED_EXTENSIONS : explode(',', CONTACT_MANAGER_UNAUTHORIZED_EXTENSIONS))));
...
elseif ((in_array($extension, $unauthorized_extensions)) || ((count($extensions) > 0) && (!in_array($extension, $extensions)))) { $GLOBALS[$prefix.$name.'_error'] = 'unauthorized_extension'; }
...
elseif ($atts['type'] != 'checkbox') { $atts['value'] = $_POST[$prefix.$name]; } }
...
elseif (isset($_SESSION['personal_informations_'.$_SERVER['REMOTE_ADDR']][$name])) { $atts['value'] = htmlspecialchars($_SESSION['personal_informations_'.$_SERVER['REMOTE_ADDR']][$name]); } } }
if (((!isset($GLOBALS['form_focus'])) || ($GLOBALS['form_focus'] == '')) && ($atts['value'] == '') && (!in_array($atts['type'], array('checkbox', 'file', 'radio')))) { $GLOBALS['form_focus'] = $prefix.$name; }
if (($atts['type'] == 'checkbox') && ((!isset($atts['checked'])) || (isset($_POST[$prefix.'submit'])) || (isset($_POST[$prefix.$name])))) { $atts['checked'] = (((isset($_POST[$prefix.$name])) && ($_POST[$prefix.$name] == $atts['value'])) ? 'checked' : ''); } }
