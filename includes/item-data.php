...
if ((!isset($GLOBALS['kleor_shortcodes_cache'])) || ($GLOBALS['kleor_shortcodes_cache'] != 'no')) {
$GLOBALS['contact_item_data_'.$type] = (array) (isset($GLOBALS['contact_item_data_'.$type]) ? $GLOBALS['contact_item_data_'.$type] : array());
$atts2 = (array) (is_string($atts) ? array($atts) : $atts);
if ((!isset($atts2['id'])) && (isset($GLOBALS[$type.'_id']))) { $atts2['id'] = $GLOBALS[$type.'_id']; }
if (isset($atts2['id'])) { $atts_key = serialize($atts2); } }
if ((isset($atts_key)) && (isset($GLOBALS['contact_item_data_'.$type][$atts_key]))) { $data = $GLOBALS['contact_item_data_'.$type][$atts_key]; }
else {
switch ($type) {
...
if ((!isset($GLOBALS['action'])) || ($GLOBALS['action'] != 'update_admin_page_form')) {
...
.' '.sprintf(__('(<a href="%1$s">More informations</a>)', 'contact-manager'), 'https://www.kleor.com/contact-manager/#'.str_replace('contact-', '', str_replace('_', '-', $type)).'-shortcodes'); } } }

foreach (array($type.'_id', $type.'_data') as $key) {
if (isset($original[$key])) { $GLOBALS[$key] = $original[$key]; } }
if (isset($atts_key)) { $GLOBALS['contact_item_data_'.$type][$atts_key] = $data; } }